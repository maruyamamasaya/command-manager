...
- `cheatsheets(id, category_id, title, description, type, content, created_at, updated_at)`
- `cheatsheet_steps(id, cheatsheet_id, sort_order, title, description, content, created_at, updated_at)`
...
`type` は `single` または `command_set`。既存行は `single` を既定値とする。Command Setは `cheatsheets 1:N cheatsheet_steps` で順序付きStepを持ち、変数は全Step本文から重複排除してCheatSheet単位で管理する。
