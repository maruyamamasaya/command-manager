  ...
  return <section className="list-panel" aria-label="CheatSheet一覧">
    ...
    <div className="cards">{items.length === 0 ? <div className="empty"><FileCode2 size={30} /><p>該当するCheatSheetはありません</p></div> : items.map((item) =>
      <article key={item.id} className={`card ${selectedId === item.id ? 'selected' : ''}`} onClick={() => onSelect(item)}>
        <div className="card-top"><div className="badges"><span className="category-badge">{item.categoryName}</span>{item.type === 'command_set' ? <span className="command-set-badge">Command Set · {item.steps.length} steps · {item.templateVariables.length} variables</span> : detectTemplateVariables(item.content).length ? <span className="template-badge">変数 {detectTemplateVariables(item.content).length}</span> : null}</div>{item.type === 'single' ? <button className="copy-button" onClick={(event) => { event.stopPropagation(); onCopy(item); }}><Copy size={14} /> Copy</button> : null}</div>
